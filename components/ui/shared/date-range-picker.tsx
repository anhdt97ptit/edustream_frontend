...
type UseDateParams = (defaultDateRange?: DefaultDateRange, shallow?: boolean) => UseDateParamsReturn
...
const useDateParams: UseDateParams = (defaultDateRange, shallow = true) => {
  ...
  const updateDateParams = (newParams: Partial<DateParams>) => {
    setDateParams((prevParams) => {
      const updatedParams: { from: string; to: string } = {
        ...prevParams,
        ...newParams,
      }
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button
            ...
            size={triggerSize}
            className={cn(
              'h-10 w-full justify-start gap-2 truncate text-left font-normal',
